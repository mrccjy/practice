...
public class TaskScheduler{
  public static void main(String[] args) {
      ...
      System.out.println(solution.leastInterval("AAABBBCCCDDE".toCharArray(), 2));
  }
...
class Solution {
    public int leastInterval(char[] tasks, int n) {
        ...
        for (int i=0; i<tasks.length; i++) {
            countMap.compute(tasks[i], (k,v) -> (v == null) ? 1: v+1);
            maxExec = Math.max(countMap.get(tasks[i]), maxExec);
        }
        int maxCount = 0;
        for (Map.Entry<Character, Integer> en : countMap.entrySet()) {
            if (en.getValue() == maxExec) {
                ++maxCount;
            }
        }
